    ...
    const { analytics, chartData, loading } = useMemo(() => {
        ...
        if (!parsedTrades || parsedTrades.length === 0) {
            ...
            const emptyAnalytics = { 
                ...
                avgWin: 0, avgLoss: 0, profitFactor: 0, maxWinStreak: 0, maxLossStreak: 0,
        ...
        chartDataPoints.push({ name: 'Başlangıç', balance: initialBalance, date: startPointDate.toISOString() });
        ...
        const winningTrades = parsedTrades.filter(t => t.pnlUsd > 0 && t.riskReward > 0);
        ...
        // En İyi ROE'yi bul
        const bestROE = Math.max(...parsedTrades.map(t => parseFloat(t.pnlPercent) || 0), 0);

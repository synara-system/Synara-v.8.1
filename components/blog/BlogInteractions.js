...
const BlogInteractions = ({ postId, postSlug, initialLikes, initialComments, T, postTitle }) => { 
    ...
    trpc.blog.getPostBySlug.useQuery(
        { slug: postSlug },
        {
            enabled: !!postSlug,
    ...
    const handleLike = async () => {
        ...
        if (hasLiked || !postId || likePostMutation.isLoading) return;
        ...
        <>
            <div className="lg:col-span-1 lg:sticky lg:top-24 h-fit space-y-6 order-2 lg:order-1">
                ...
                <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 space-y-3">
                    ...
                    <button 
                        onClick={handleLike} 
                        disabled={!isUserLoggedIn || hasLiked || likePostMutation.isLoading}
                        ...
                        title={!isUserLoggedIn ? "Oturum Açın" : (hasLiked ? "Zaten Onayladınız" : "Yazıyı Beğen")}
                    >

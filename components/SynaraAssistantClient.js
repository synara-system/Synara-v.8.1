...
const SynaraAssistantClient = () => {
    ...
    // BU useEffect DOĞRU VE STABİL (Scroll Logic)
    useEffect(() => {
        const container = messagesContainerRef.current;
        if (container) {
    ...
    // BU useEffect DÜZELTİLDİ (v1.4) (Firestore Listener Logic)
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // const prevMessageCount = messages.length; // KALDIRILDI (v1.4 - Hata Kaynağı)
            const newMessageCount = fetchedMessages.length; // Hoşgeldin mesajı için GEREKLİ

            setMessages(fetchedMessages);
            
            // Hoşgeldin mesajı mantığı (OK)
            ...
            // ÇAKIŞAN SCROLL MANTIĞI KALDIRILDI (v1.4)
        ...
        return () => unsubscribe();
    // KRİTİK FİX (v1.4): Bağımlılık dizisi stabilize edildi.
    // 'messages.length' (race condition) kaldırıldı.
    // 'showAlert' (unstable dependency) ve 'scrollToBottom' (stable) korundu.
    }, [user, showAlert, scrollToBottom]); 

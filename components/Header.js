...
const MobileNavItem = ({ children, onClick, href, className, closeMenu, iconName }) => {
    ...
    const isMestegLink = href === '/about-us' || href === '/vision';
...
const MestegDropdown = ({ T, pathname, closeMobileMenu }) => {
    ...
    const isActive = pathname.startsWith('/about-us') || pathname.startsWith('/vision');
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10, scale: 0.95 }}
                        transition={{ duration: 0.15, ease: "easeOut" }}
                        className="absolute left-1/2 -translate-x-1/2 mt-2 w-56 origin-top bg-gray-900/95 backdrop-blur-lg rounded-xl shadow-2xl ring-1 ring-amber-500/30 p-2"
                        role="menu"
                        aria-labelledby={buttonId}
                    >
                        <div className="p-1 space-y-1">
...
// Ana Header Bileşeni
const Header = () => {
    const { T, user, isAdmin, handleLogout: authHandleLogout } = useAuth();
    const pathname = usePathname(); 
    ...
    }, [authHandleLogout, router, queryClient]);
    ...
    const isRootPage = pathname === '/';
    const isMestegPage = pathname.startsWith('/about-us') || pathname.startsWith('/vision');
    ...
    const NavItem = ({ href, children, className, onClick }) => {
        ...
        const isMestegLink = finalHref === '/about-us' || finalHref === '/vision';
    ...
    return (
        <header ref={headerRef} className={headerWrapCls}>
            ...
            <div 
                id="mobile-menu-list"
                className={`md:hidden bg-[#0b1220]/95 backdrop-blur-sm border-b border-white/5 overflow-y-auto transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-screen' : 'max-h-0'}`}
                role="menu"
            >

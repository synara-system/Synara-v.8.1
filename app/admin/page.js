...
const AdminIndexPage = () => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        return allUsers.filter(u =>
            ...
            u.tradingViewUsername.toLowerCase().includes(query)
        );
        ...
        <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8">
            <div className="container mx-auto max-w-6xl">
                 <div className="flex justify-between items-center mb-8">
                    <h1 className="text-3xl md:text-4xl font-bold">{T.admin_title}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <Link href="/admin/blog-editor" className={`group flex flex-col justify-between p-6 rounded-2xl border border-indigo-700/50 bg-indigo-900/20 text-indigo-400 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl`}>
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-lg bg-indigo-900/70`}>
                                <Icon name="pencil" className={`w-6 h-6 text-indigo-400`} />
                            </div>
                            <h3 className="text-xl font-bold text-white">İçerik Yönetimi</h3>
                        </div>
                        <p className="text-sm text-gray-400 mt-4">Synara Blog ve içerik düzenlemelerini yönetin.</p>
                    </Link>
                    <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 shadow-xl flex items-center justify-center">
                        <h3 className="text-xl font-bold text-white">Toplam Kayıtlı Kullanıcı:
                            <span className="ml-2 text-indigo-400">{filteredUsers.length}</span>
                        </h3>
                    </div>
                </div>

...
const DEFAULT_CATEGORIES = ['Analiz', 'Eğitim', 'Güncelleme', 'Haberler'];
...
const getCategoryStyles = (category, themeColor) => {
    ...
    if (themeColor === 'orange') {
        ...
        // KRİTİK NİHAİ FİX: Active buton için text-white zorlanıyor.
        const activeCls = `relative text-white border-2 border-${baseColor}-400/80 bg-${baseColor}-900/40`; 
        
        // KRİTİK DÜZELTME: Inactive butonlar için text-white ve hover'da hafif koyulaşma.
        const inactiveCls = 'bg-gray-800 text-white hover:bg-gray-700/80 border border-gray-700';
...
const BlogIndexClient = ({
    ...
    pageTitle,
    pageSubtitle,
    // KRİTİK DEĞİŞİKLİK: İlk kategori varsayılan olarak DEFAULT_CATEGORIES'in ilk elemanı olacak.
    ...
    // KRİTİK DEĞİŞİKLİK: Başlangıç kategorisi artık 'Tümü' yerine customCategories[0]'dır.
    const [activeCategoryKey, setActiveCategoryKey] = useState(customCategories[0]?.key || DEFAULT_CATEGORIES[0]);
    ...
    const dynamicCategories = useMemo(() => {
        ...
        // KRİTİK DEĞİŞİKLİK: 'Tümü' dinamik kategori listesinden kaldırıldı.
        const allCats = [...new Set(rawPosts.map(p => p.category).filter(Boolean))];
    ...
    const filteredPosts = useMemo(() => {
        ...
        if (activeCategoryKey === (T?.news_category_all || 'Tümü')) {
             if (isNewsMode) {
                 return basePosts.filter(p => allowedNewsCategories.includes(p.category)); // Tüm Haberler
             } else {
                 return basePosts.filter(p => allowedBlogCategories.includes(p.category)); // Tüm Bloglar
             }
        }
        ...
        <div className="w-full">
            ...
            >
                ...
                <div className="container mx-auto px-6 relative z-10">
                    ...
                    {/* KRİTİK DÜZELTME: Kategori butonlarının hizalanması */}
                    {dynamicCategories.length > 0 && (
                        // KRİTİK DEĞİŞİKLİK: bg-black/80 ve p-3 sınıfları KALDIRILDI.
                        <div className="flex justify-center flex-wrap gap-3 pt-4 mx-auto max-w-fit overflow-hidden">
                            {dynamicCategories.map(category => {
                                
                                // KRİTİK FİX: Tümü butonu için özel stil (turuncu lekeyi kesmek için)
                                const isAllTab = category.key === (T?.news_category_all || 'Tümü');
                                ...
                                // Tümü butonu aktifse, sadece border ve white text kullan (kaybolan renkten kaçınmak için)
                                const finalButtonCls = isAllTab && activeCategoryKey === category.key 
                                    ? `px-5 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 relative text-white border-2 border-orange-400/80 bg-orange-900/40 transform-none shadow-none` 
                                    : finalCls;

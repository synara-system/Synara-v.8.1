...
const calculateReadingTime = (htmlContent) => {
    ...
    // KRİTİK DÜZELTME: HTML etiketlerini temizleyerek kelime sayısını bul
    const noOfWords = htmlContent.replace(/<[^>]+>/g, '').trim().split(/\s/g).length;
...
const BlogArticle = ({ post }) => {
    ...
    // Ham metin üzerinden okuma süresini hesapla (post.content artık HTML olduğu varsayılıyor)
    ...
    return (
        <article className="bg-gray-900 p-8 md:p-12 rounded-2xl border border-gray-700 shadow-2xl shadow-indigo-900/20">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-4 mb-8 text-sm text-gray-400">
                ...
                {/* KRİTİK DÜZELTME: post.createdAt artık ISO string olarak bekleniyor */}
            ...
            <div
                // Client Component'ten gelen HTML içeriği doğrudan buraya gömülür.

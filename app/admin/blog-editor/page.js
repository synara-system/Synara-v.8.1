...
const AdminBlogEditorIndexPage = () => {
    ...
    // v1.0.2 SIRA DÜZELTMESİ: 'allPosts' useMemo içine alındı ve sıralandı.
    const filteredPosts = useMemo(() => {
        const allPosts = postsData || [];

        // v1.0.2 - KÖK NEDEN DÜZELTMESİ:
        // Sunucudan .orderBy kaldırıldığı için, client'ta (burada) sıralama zorunludur.
        // En yeni (createdAt) olan üste gelecek şekilde sırala.
        const sortedPosts = [...allPosts].sort((a, b) => {
            // Güvenlik: Tarih objelerinin geçerli olduğundan emin ol
            const dateA = a.createdAt ? new Date(a.createdAt).getTime() : 0;
            const dateB = b.createdAt ? new Date(b.createdAt).getTime() : 0;
            return dateB - dateA; // b - a = Azalan (desc) sıra
        });

        if (!searchQuery) return sortedPosts; // Sıralanmış listeyi döndür

        const query = searchQuery.toLowerCase();
        // Sıralanmış liste üzerinde filtreleme yap
